---
title: "From RTF 3.5 to RTF 1.26: Optimizing a Pure C TTS Engine"
date: 2026-03-16
description: How cache alignment, SIMD intrinsics, pipeline threading, and algorithm fixes nearly tripled inference speed in a pure C text-to-speech engine
---

![Qwen-TTS optimizations cover](/static/qwen-tts-optimizations.png)
...
We have a pure C inference engine for [Qwen3-TTS](https://github.com/gabriele-mastrapasqua/qwen3-tts), a text-to-speech model with
...
**Result: 93ms → 21ms (4.4x faster).** Output bit-identical — same threshold,
same filtering, same samples. The only thing that changed was how fast we find
the threshold value.

We also checked softmax (3 scalar passes over vocab) and top-p (O(n^2) full
...
has `hidden=2048` and `intermediate=6144` — 4x larger matrices. Time to
revisit.
...
**Softmax SIMD vectorization** (est. 2-4x, actual: not worth it). After
...
**Speech decoder depthwise conv / LayerNorm SIMD** (est. 1.5-3x, actual: not
...
worth it). Only 5 layers x 264 bytes = 1.3KB total. The bottleneck is the
...
7. **Check your algorithms, not just your SIMD.** A 4x sampling speedup from
